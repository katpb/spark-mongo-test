	...
	public Image getImage() {
		...
		if (imageBytes == null && imageStrips != null) {
			...
			for (i=0,n=0,rows=0,lastRow=imageStrips.length-1 ; i<imageStrips.length; i++, rows+=rowsPerStrip) { //imageStrips.length
				...
				byte rawStrip[] = DecodeImageStrip( imageStrips[i], (i==lastRow ? (imageHeight-rows) : rowsPerStrip), true); //(i==0));
	...
	byte [] DecodeImageStrip(byte[] imageStrip, int maxLines, boolean firstStrip)
	{
		...
		if (firstStrip) {
            ...
            if (logger.isTraceEnabled())
            {
                for (i=0; i<8; i++)
                {
                    logger.trace( " {}", Converter.byteToBinaryString( bytesArray[i] ) );
                }
            }
		...
		for (lines=1,count=bytesArray.length*8; lines<maxLines && i<count; ) {
					...
					if (nPixels>=rawImage.length || nPixels+code.runLength>=rawImage.length || code.runLength>=WhiteRun.length || code.runLength>=BlackRun.length) {
                        if (logger.isInfoEnabled())
                        {
                            logObjs = new Object[]
                            {
                                ...
                                Integer.valueOf(code.runLength),
                                Integer.valueOf(rawImage.length),
                                Integer.valueOf(WhiteRun.length),
                                Integer.valueOf(BlackRun.length)
                            };
                            logger.info("nPixels={}, code.runLength={}, rawImage.length={}, WhiteRun.length={}, BlackRun.length={}", logObjs);
                        }
					}
					System.arraycopy( (whiteRun ? WhiteRun : BlackRun), 0, rawImage, nPixels, code.runLength );
            ...
            if (logger.isTraceEnabled())
            {
                logger.trace("{}{}", (whiteRun ? " W" : " B"), Integer.valueOf(code.runLength));
                if (code.runLength<0)
                {
                    logger.trace("{}", code);
                }
        ...
        if (logger.isTraceEnabled())
        {
            ...
            logger.trace("nPixels = {}, rawImage.length={}, lines={}, maxLines={}, imageWidth={}");
        }
