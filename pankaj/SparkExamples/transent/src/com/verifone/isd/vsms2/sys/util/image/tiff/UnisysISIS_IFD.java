...
import com.verifone.isd.vsms2.sys.util.image.tiff.tag.UnisysISISTags;
...
class UnisysISIS_IFD extends IFD {
	...
	public String toString() {
		...
		for (i=0; i<count; i++) {
			...
			s += "\tTag: " + ut.toString()
				 + ", Type: " + entries[i].type.toString() + ", Count: " + entries[i].count;
					
			if (entries[i].isOffset()) {
				int len = Math.min( entries[i].dataArray.length, 256 );
//				s += ", Offset: 0x" + Long.toHexString(entries[i].value) + ", " + entries[i].sizeOfData() + "bytes";
				
				if (entries[i].type.isAscii()) {
					String sz = new String (entries[i].dataArray); //jdk1.1 allows String(byte[])
					s += "\n\t\tValue:"+sz + "\n";
				}
			}
