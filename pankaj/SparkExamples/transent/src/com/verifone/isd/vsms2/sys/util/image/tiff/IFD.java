...
class IFD {
    ...
    private Logger logger = LoggerFactory.getLogger(IFD.class);
	...
	public void writeImageBytes(MotorolaIntelInputFilter out, int nOffsets, int nCounts) throws IOException {
		...
		if (entries[nCounts].isOffset()) {
			logger.warn("No I won't write out in strips for now");
		}
		else { //just one strip
	...
	public void readImageBytes(MemoryFileInputFilter in, int nOffsets, int nCounts) {
		...
		if (nCounts != 0 && entries[nCounts].isOffset()) {
			...
			len = (nCounts>0 ? (int)entries[nCounts].value : (int)( in.length()-offset ));
