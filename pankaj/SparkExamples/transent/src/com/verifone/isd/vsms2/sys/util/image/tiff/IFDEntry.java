...
import com.verifone.isd.vsms2.sys.util.image.tiff.util.MemoryFileInputFilter;
import com.verifone.isd.vsms2.sys.util.image.tiff.util.MotorolaIntelInputFilter;
...
class IFDEntry {
	...
	public void read( MemoryFileInputFilter in, long ifdOffset )  {
		...
		if (isOffset()) {
			...
			if (type.isAscii())  in.readFully( dataArray ); 
		...
		if (type.isShort() && !in.isIntel())
			value = (int)( ((long)value>>16)&0xffff );
	}
	
	public String toString() {
		String s;
		s = "Tag: " + tag.toString() + ", Type: " + type.toString() + ", Count: " + count;
		...
		if (tag.equals(Tag.COMPRESSION)) 
			...
			s += ", Offset: 0x" + Long.toHexString(value) + ", " + sizeOfData() + "bytes";
			
			if (type.isAscii()) {
				String sz; // = new String (dataArray); //jdk1.1 allows String(byte[])
				sz = new String(dataArray, 0, dataArray.length); //jdk1.02 allows String(byte ascii[], int hibyte, int offset, int count)
				s += "\n\t\t" + sz + "\n";
			}
			else if (type.isRational()) {
				int n = ((dataArray[0]&0xff)<<24) + ((dataArray[1]&0xff)<<16) + ((dataArray[2]&0xff)<<8) + (dataArray[3]&0xff);
				int d = ((dataArray[4]&0xff)<<24) + ((dataArray[5]&0xff)<<16) + ((dataArray[6]&0xff)<<8) + (dataArray[7]&0xff);
				s += "\n\t\t" + n + "/" + d + "\n";
			}
...
//					int d = (((int)dataArray[i])&0xff) + ((((int)dataArray[i+1])&0xff)<<8); 
//					int d = ((((int)dataArray[i])&0xff)<<8) + (((int)dataArray[i+1])&0xff); 
...
//					s += Integer.toHexString( d ); 
				...
				for (i=0; i<len; i+=4) {
					int n = ((dataArray[i]&0xff)<<24) + ((dataArray[i+1]&0xff)<<16) + ((dataArray[i+2]&0xff)<<8) + (dataArray[i+3]&0xff);
					if (i%16==0) s += "\n\t\t";
					else s += ", "; 
					s += n;
				}
...
/******************gets ugly************
				...
				for (i=0; i<len; i++) {
					...
					s += Integer.toHexString( dataArray[i]&0xff ); 
				}
				s += "\n";
			}
***************************************/
