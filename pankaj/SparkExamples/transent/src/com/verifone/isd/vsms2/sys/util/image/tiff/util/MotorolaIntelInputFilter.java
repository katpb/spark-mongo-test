...
/**
 * MotorolaIntelInputFilter is basically RandomAccessFile, but can be told
 * which byte order system to use when reading and writing files.
...
public class MotorolaIntelInputFilter {
    ...
    private Logger logger = LoggerFactory.getLogger(MotorolaIntelInputFilter.class);
    ...
    public void writeByte( int v ) throws IOException {
		...
		public void writeInt(int v) throws IOException {
				...
				if (isIntel) {
					b1 = (int)((v&0xff000000)>>24);
					b2 = (int)((v&0x00ff0000)>>16);
					b3 = (int)((v&0x0000ff00)>>8);
					b4 = (int) (v&0x000000ff);
    ...
    public void readFully(byte[] b, int off, int len) throws IOException {
    	//may want to change this to flip the bytes as we read them raf.
            logger.trace("readFully( bytes, {}, {} )", Integer.valueOf(off), Integer.valueOf(len));
			int n = 0;
			while (n < len) {
    		...
    		while ( n < len ) {
	    		switch (typeSize) {
  	  			case 2: {
								...
								bytes[n] = (byte) b2;
								bytes[n+1] = (byte) b1;
                                logger.trace("Read a short {}{} ", Byte.valueOf(bytes[n]), Byte.valueOf(bytes[n+1]));
								break;
							}
  	  			case 4: {
								b1 = raf.readUnsignedByte();
