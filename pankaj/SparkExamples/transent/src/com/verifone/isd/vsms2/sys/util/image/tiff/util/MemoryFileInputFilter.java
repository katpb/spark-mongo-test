...
public class MemoryFileInputFilter {
     ...
     * false. Any other value represents true.
		...
		/**
     * Reads a Unicode character from this byte array. The method reads 2
     * bytes from this byte array in the proper byte order. If the two bytes read, in the proper byte order, are
     ...
     *     (char)((b1 &lt;&lt; 8) | b2)
     * </pre></blockquote>
          * @return    the next two bytes of this byte array, interpreted as a Unicode character.
     */
		public char readChar() {
				...
				if (isIntel) 	c = (char)(((h&0xff)<<8) + (l&0xff));
				else 					c = (char)(((l&0xff)<<8) + (h&0xff));

				return c;
		}

		/**
     * Reads an unsigned 8-bit number from this byte array. This method reads a byte from this byte array and returns that byte.
          * @return    the next byte of this byte array, interpreted as an unsigned 8-bit number.
     */
     ...
     *     (short)((b1 &lt;&lt; 8) | b2)
     * </pre></blockquote>
          * @return    the next two bytes of this byte array, interpreted as a signed 16-bit number.
     */
		public short readShort() {
				...
				if (isIntel) 	s = (short)(((h&0xff)<<8) + (l&0xff));
				else 					s = (short)(((l&0xff)<<8) + (h&0xff));
     ...
     *     (b1 &lt;&lt; 8) | b2
     * </pre></blockquote>
          * @return    the next two bytes of this byte array, interpreted as an unsigned 16-bit number.
     */
		public int readUnsignedShort() {
				...
				if (isIntel) 	s = (((h&0xff)<<8) + (l&0xff));
				else 					s = (((l&0xff)<<8) + (h&0xff));
     ...
     * bytes from the byte array. If the bytes read, in the proper byte order, are <code>b1</code>,
     ...
     */
		public long readInt() {
				...
				if (isIntel) 	i = (((b4&0xff)<<24) + ((b3&0xff)<<16) + ((b2&0xff)<<8) + (b1&0xff));
				else 					i = (((b1&0xff)<<24) + ((b2&0xff)<<16) + ((b3&0xff)<<8) + (b4&0xff));
				return i;
		}

		/**
     * Reads a signed 64-bit integer from this byte array. This method reads 8
     * bytes from the byte array. If the bytes read, in the proper byte order, are <code>b1</code>, <code>b2</code>, <code>b3</code>,
     ...
     */
		public long readLong() {
				...
				if (isIntel)
					i = ((long)b8 << 56) + ((long)b7 << 48)
               + ((long)b6 << 40) + ((long)b5 << 32)
               + ((long)b4 << 24) + ((long)b3 << 16)
               + ((long)b2 << 8) + b1;
				else
					i = ((long)b1 << 56) + ((long)b2 << 48)
               + ((long)b3 << 40) + ((long)b4 << 32)
               + ((long)b5 << 24) + ((long)b6 << 16)
               + ((long)b7 << 8) + b8;
     ...
     * using the <code>longBitsToDouble</code> method in class
     * <code>Double</code>.
     ...
     */
		...
		/**
		 * Reads up to b.length bytes of data from this byte array into an array of bytes.
				* @param b the buffer into which the data is copied
		 		* @return the total number of bytes copied into the buffer, or -1 if there is no more data because the end of this byte array has been reached.
		 */
		...
		/**
		 * Reads up to len bytes of data from this byte array (after incrementing the current index by off) into an array of bytes.
				* @param b the buffer into which the data is copied
    ...
    public void readFully(byte[] b, int off, int len) {
		...
		/**
		 * Reads up to len bytes of data from this byte array into an array of bytes. If our byte array file is in motorola format or the typeSize < 2 then it just does a standard array copy for len bytes.
         * @param bytes the buffer into which the data is copied
         * @param typeSize the size of data type (ie SHORT_SIZE, ...).
         * @param len the maximum number of bytes copied.
		 */
