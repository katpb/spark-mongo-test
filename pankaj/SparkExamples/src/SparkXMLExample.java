...
import static org.apache.spark.sql.functions.explode;public class SparkXMLExample {
...
//	    implicitDS.select("trans.trHeader.trUniqueSN").show();
			...
			Dataset<Row> trLines = implicitDS.select(col("trans.trHeader.trUniqueSN"), col("trans.type"),
			...
			Dataset<Row> trPayLines = implicitDS.select(col("trans.trHeader.trUniqueSN"), col("trans.type"),
...
//	    creditRefundCount.show();
			Dataset<Row> refundTrans = implicitDS.filter(col("trans.type").contains("refund"))
...
//	    dep2details.show(); 
			// Dataset<Row> trGt5 = sparkSession.sql("SELECT trans.trHeader.trUniqueSN FROM
			// transactions where trans.trValue.trTotWTax= 5.00");
